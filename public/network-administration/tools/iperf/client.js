document.addEventListener('DOMContentLoaded', function() {
    ...
    function init() {
        ...
        console.log("Checking DOM elements: sourceServerSelect", !!sourceServerSelect);
        console.log("Checking DOM elements: destinationServerSelect", !!destinationServerSelect);
        console.log("Checking DOM elements: discoverServersBtn", !!discoverServersBtn);
        ...
        if (serversStatus) {
            ...
            serversStatus.textContent = 'Click "Discover Servers" to find iperf servers on your network';
        } else {
            console.error("serversStatus element not found - discovery progress won't be visible");
        }
        ...
        setTimeout(() => {
            ...
            if (serversStatus) {
                serversStatus.textContent = 'Default local server added. Click "Discover Servers" to find more.';
            }
        }, 1000);
    }
        ...
        fetch('/api/network-administration/inventory/proxy-servers')
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (socket && socket.readyState === WebSocket.OPEN && discoveredServers.length === 0) {
                        setTimeout(() => {
    ...
    function populateServerSelects(servers) {
        ...
        sourceServerSelect.innerHTML = '<option value="">Select source server</option>';
        destinationServerSelect.innerHTML = '<option value="">Select destination server</option>';
        ...
        if (sourceValue && Array.from(sourceServerSelect.options).some(opt => opt.value === sourceValue)) {
            sourceServerSelect.value = sourceValue;
        }
        
        if (destValue && Array.from(destinationServerSelect.options).some(opt => opt.value === destValue)) {
    ...
    function setupEventListeners() {
        ...
        if (discoverServersBtn) {
            ...
            discoverServersBtn.addEventListener('click', function() {
                ...
                if (window.debugLog) {
                    window.debugLog("DISCOVER BUTTON CLICKED FROM CLIENT.JS HANDLER!", "info");
                    window.debugLog("WebSocket State: " + (socket ? socket.readyState : "no socket"), "info");
                ...
                if (serversStatus) {
                    serversStatus.className = 'alert alert-info';
                    serversStatus.textContent = 'Discovering iperf servers on your network... (click started)';
                    
                    if (window.debugLog) {
                        window.debugLog("Updated serversStatus: " + serversStatus.textContent, "info");
                    }
                } else {
                ...
                setTimeout(function() {
                    if (isDiscovering) {
                        ...
                        if (serversStatus) {
                            ...
                            serversStatus.textContent = 'Found 1 iperf server (local fallback)';
                        }
        ...
        sourceServerSelect.addEventListener('change', function() {
            if (sourceServerSelect.value === destinationServerSelect.value && sourceServerSelect.value !== '') {
                destinationServerSelect.value = '';
            }
        });
        
        destinationServerSelect.addEventListener('change', function() {
            if (destinationServerSelect.value === sourceServerSelect.value && destinationServerSelect.value !== '') {
        ...
        if (directionSelect) {
            directionSelect.addEventListener('change', function() {
                ...
                const destLabel = document.querySelector('label[for="destinationServer"]');
    ...
    function discoverIperfServers(forceRefresh = false) {
        ...
        if (window.debugLog) {
            window.debugLog("discoverIperfServers function called with forceRefresh = " + forceRefresh, "info");
        }
        
        if (!socket) {
            console.error("Socket is null or undefined");
            if (window.debugLog) {
                window.debugLog("Socket is null or undefined - WebSocket not initialized", "error");
            }
        ...
        if (socket.readyState !== WebSocket.OPEN) {
            ...
            showError('WebSocket connection not available (state: ' + socket.readyState + ')');
            return;
        }
        ...
        setTimeout(() => {
            if (isDiscovering) {
                ...
                if (serversStatus) {
                    ...
                    serversStatus.textContent = 'Server discovery timed out. The server may be busy or unable to scan the network.';
                }
            }
    ...
    function setupWebSocket() {
        ...
        socket.onerror = function(error) {
            ...
            if (serversStatus) {
                ...
                serversStatus.textContent = 'WebSocket connection error. Please check the server status.';
            }
        };
    }
    ...
    function handleWebSocketMessage(data) {
        switch (data.type) {
            ...
            case 'discoveryComplete':
                ...
                if (serversStatus) {
                    if (discoveredServers.length > 0) {
                        ...
                        serversStatus.className = 'alert alert-warning';
                        serversStatus.textContent = 'No iperf servers found on your network. Make sure iperf3 is running on target machines.';
                    }
                }
                break;
            ...
            case 'discoveryError':
                ...
                if (serversStatus) {
                    serversStatus.className = 'alert alert-danger';
                    serversStatus.textContent = 'Error discovering servers: ' + data.message;
                }
                break;
