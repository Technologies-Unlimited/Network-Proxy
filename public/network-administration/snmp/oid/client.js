...
function useMIBBrowser() {
  ...
  const commonMibs = [
    ...
    },
    {
      name: 'IF-MIB',
      oid: '1.3.6.1.2.1.31',
      description: 'Extended interface info',
    },
  ]
  ...
  const browseMIB = async (oidPath = '1.3.6.1.2.1.1') => {
    ...
    try {
      ...
      if (!isServerRunning) {
        throw new Error(
          'Server is not running. Please start the server with "bun --hot app.ts" command.'
        )
      }
      ...
      // For template-based browsing
        ...
        if (template) {
          ...
          if (template.version === '1' || template.version === '2c') {
            ...
            if (
              template.v3SecurityLevel === 'authNoPriv' ||
              template.v3SecurityLevel === 'authPriv'
            ) {
...
function useSNMPOIDData(companyId = 'default-company-id') {
      ...
      const mockOids = [
        {
          ...
          dataType: 'String',
          description:
            'A textual description of the device including manufacturer name and operating system.',
          category: 'System',
          units: '',
  ...
  const updateFormWithMIBData = mibData => {
    ...
    if (existingOid) {
      console.log(
        `OID ${mibData.oid} already exists as "${mibData.existingName || 'Unknown'}"`
      )
    ...
    setFormData(prev => ({
      ...
      description:
        mibData.existingDescription || mibData.description || prev.description,
  ...
  const filteredOIDs = oids.filter(
    oid =>
...
const SNMPOID = () => {
  ...
  return h(
    ...
    h(
      ...
      h('h1', { style: { margin: '0', color: '#333' } }, 'SNMP OID Management'),
      h(
        'div',
                ...
                h(
                  'button',
                  {
                    ...
                    style: {
                      padding: '8px 16px',
                      backgroundColor:
                        browseMethod === 'direct' ? '#e3f2fd' : 'transparent',
                      border: 'none',
                      borderBottom:
                        browseMethod === 'direct'
                          ? '2px solid #1976d2'
                ...
                h(
                  'button',
                  {
                    ...
                    style: {
                      padding: '8px 16px',
                      backgroundColor:
                        browseMethod === 'template' ? '#e3f2fd' : 'transparent',
                      border: 'none',
                      borderBottom:
                        browseMethod === 'template'
                          ? '2px solid #1976d2'
                          : 'none',
                      cursor: 'pointer',
                      fontWeight:
                        browseMethod === 'template' ? 'bold' : 'normal',
                    },
                  },
                  'Use Device Template'
                )
                  ...
                  selectedDevice &&
                    h(
                      ...
                      h(
                        ...
                        templates.find(t => t.id === selectedDevice.templateId)
                          ?.name
                      ),
                      h(
                        'p',
                  ...
                  h(
                    'button',
                    {
                      ...
                      style: {
                        ...
                        borderRadius: '4px',
                        cursor:
                          mibLoading || !browsePath ? 'not-allowed' : 'pointer',
                    ...
                    mibData.map((oid, index) =>
                      h(
                        'tr',
                        {
                          ...
                          style: {
                            backgroundColor:
                              selectedOid === oid ? '#e3f2fd' : 'transparent',
                            borderBottom: '1px solid #ddd',
                            cursor: 'pointer',
                          },
                        ...
                        h(
                          ...
                          h(
                            'button',
                            {
                              ...
                              title: oid.existsInDatabase
                                ? 'View/edit existing OID'
                                : 'Import as new OID',
                              style: {
                                padding: '4px 8px',
                                backgroundColor: oid.existsInDatabase
                                  ? '#2196f3'
                                  : '#4caf50',
                                color: 'white',
                                border: 'none',
          ...
          h(
            ...
            h(
              ...
              h('input', {
                type: 'text',
                placeholder:
                  'Search OIDs by name, OID string, description, or category...',
                value: searchTerm,
                ...
                oids.length === 0
                  ...
                  : oids.map(oid => {
                      return h(
                        ...
                        h('td', { style: { padding: '12px' } }, oid.name),
                        h(
                          'td',
                          {
                            style: {
