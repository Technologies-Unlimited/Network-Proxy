...
export async function discoverActiveHosts(subnet: string): Promise<string[]> {
  ...
  if (numHosts > 254) {
    console.warn(
      `Large subnet detected (${numHosts} hosts). Limiting scan to first 254 hosts.`
    )
  }
    ...
    if (isWindows) {
      ...
      try {
        const { stdout } = await execAsync(
          `nmap -sn ${subnet} | grep "Nmap scan report for"`
        )
        const matches = stdout.match(
          /Nmap scan report for\s+(\d+\.\d+\.\d+\.\d+)/g
        )
        if (matches) {
          return matches
  ...
  for (const host of hosts) {
    try {
      ...
      if (isWindows) {
        ...
        try {
          const { stdout } = await execAsync(
            `powershell -command "Test-NetConnection -ComputerName ${host} -Port ${port} -InformationLevel Quiet -WarningAction SilentlyContinue"`
          )
 ...
 * @returns List of discovered iperf3 servers
 */
export async function discoverAllIperfServers(
  port: number = 5201
): Promise<DiscoveredService[]> {
  try {
    ...
    interfaces.forEach(iface => {
      console.log(
        `Interface: ${iface.name}, Address: ${iface.address}, Mask: ${iface.netmask}`
      )
    })
    ...
    for (const iface of interfaces) {
      try {
        ...
        console.error(
          `Error scanning subnet for interface ${iface.name}:`,
          error
        )
      }
    }
    ...
    console.log(
      `Found ${uniqueHosts.length} unique active hosts across all interfaces`
    )
