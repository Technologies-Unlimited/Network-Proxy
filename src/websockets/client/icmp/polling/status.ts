...
export function useICMPPollingStatus(companyId: string) {
  const [icmpPollingStatuses, setIcmpPollingStatuses] = useState<ICMPPollingStatus[]>([]);
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      if (icmpPollingStatuses.length > 0 && isConnected) {
        const randomIndex = Math.floor(Math.random() * icmpPollingStatuses.length);
        const updatedStatuses = [...icmpPollingStatuses];
        const device = { ...updatedStatuses[randomIndex] };
        
        // Flip status occasionally
        if (Math.random() > 0.8) {
          device.deviceStatus = device.deviceStatus === 'online' ? 'offline' : 'online';
        }
