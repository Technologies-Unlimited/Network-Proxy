/**
 * ICMP Polling Template Repository
 * Provides data access methods for ICMP polling templates using SQLite
 */

import { Database } from 'bun:sqlite'
import { getDatabase, generateId } from '../../../index'
import {
  icmpPollingTemplateTableSchema,
...
export class ICMPPollingTemplateRepository {
  private db: Database
  ...
  /**
   * Initializes the database tables for ICMP polling templates
   */
  initTables(): void {
    // Execute the schema creation SQL
    this.db.run(icmpPollingTemplateTableSchema)
  }
  ...
  async getForCompany(companyId: string): Promise<ExtendedICMPPollingTemplateFields[]> {
    ...
    // Map the database results to the ExtendedICMPPollingTemplateFields interface
  ...
  /**
   * Update an existing ICMP polling template
   */
  update(
    _id: string,
    companyId: string,
    ...
    let updateSql = 'UPDATE icmp_polling_templates SET '
    const params: any[] = []
  ...
  /**
   * Delete a ICMP polling template
   */
