/**
 * SNMPv3 Polling Template Repository
 * Provides data access methods for SNMPv3 polling templates using SQLite
 */

import { Database } from 'bun:sqlite'
import { getDatabase, generateId } from '../../../../index'
import {
  snmpv3PollingTemplateTableSchema,
  ExtendedSNMPv3PollingTemplateFields,
  timeIntervalToColumns,
  columnsToTimeInterval,
} from '@/schema/network-administration/snmp/polling/template/snmpv3/schema'

export class SNMPv3PollingTemplateRepository {
  private db: Database
  ...
  /**
   * Initializes the database tables for SNMPv3 polling templates
   */
  initTables(): void {
    // Execute the schema creation SQL
    this.db.run(snmpv3PollingTemplateTableSchema)
  }

  /**
   * Get all SNMPv3 polling templates for a company
   */
  async getForCompany(companyId: string): Promise<ExtendedSNMPv3PollingTemplateFields[]> {
    ...
    // Map the database results to the ExtendedSNMPv3PollingTemplateFields interface
  ...
  /**
   * Get a single SNMPv3 polling template by ID
   */
  getById(_id: string, companyId: string): ExtendedSNMPv3PollingTemplateFields | null {
  ...
  /**
   * Get a SNMPv3 polling template by name
   */
  getByName(name: string, companyId: string): ExtendedSNMPv3PollingTemplateFields | null {
   ...
   */
  create(companyId: string, input: Partial<ExtendedSNMPv3PollingTemplateFields>): ExtendedSNMPv3PollingTemplateFields {
    ...
    // Insert the SNMPv3 polling template
    this.db.query(`
      INSERT INTO snmpv3_polling_templates (
        _id, company_id, name, description, frequency, timeout, retries,
        polling_frequency_days, polling_frequency_hours, 
        polling_frequency_minutes, polling_frequency_seconds,
        downtime_trigger_days, downtime_trigger_hours, 
  ...
  /**
   * Update an existing SNMPv3 polling template
   */
  update(
    _id: string,
    companyId: string,
    input: Partial<ExtendedSNMPv3PollingTemplateFields>
  ): ExtendedSNMPv3PollingTemplateFields | null {
    ...
    let updateSql = 'UPDATE snmpv3_polling_templates SET '
    const params: any[] = []
  ...
  /**
   * Delete a SNMPv3 polling template
   */
  delete(_id: string, companyId: string): boolean {
    ...
    // Delete the template
    this.db.query(`
      DELETE FROM snmpv3_polling_templates
      WHERE _id = ? AND company_id = ?
    `).run(_id, companyId)
