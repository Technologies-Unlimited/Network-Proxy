  ...
  try {
    server = serve({
        ...
        '/api/iperf/start': {
          async POST(req) {
            ...
            try {
              ...
              if (
                !validateTarget(sourceServer) ||
                !validateTarget(destServer)
              ) {
              ...
              console.log(
                `Starting iperf test from ${sourceServer} to ${destServer}`
              )
            ...
            try {
              ...
              return Response.json({
                ...
                message: result
                  ...
                  : 'Failed to stop test or test not found',
              })
            } catch (error) {
                ...
                async start(controller) {
                  ...
                  for (let i = 0; i < maxPings; i++) {
                    ...
                    try {
                      ...
                      if (i < maxPings - 1) {
                        console.log(
                          'Stream adding 1 second delay after error...'
                        )
                        await new Promise(resolve => setTimeout(resolve, 1000))
                      }
                    }
                  ...
                  try {
                    ...
                    const args = isWindows
                      ...
                      : [
                          ...
                          '1',
                          target,
                        ] // Added -q 1 for single query per hop
                    ...
                    tracerouteProcess.stdout?.on('data', (data: Buffer) => {
                      ...
                      for (const line of lines) {
                        ...
                        console.log(
                          `Processing traceroute line: ${trimmedLine}`
                        )
            ...
            try {
              ...
              if (endPortNum - startPortNum > 10000) {
                ...
                return Response.json(
                  {
                    error:
                      'Port range too large. Maximum 10,000 ports allowed.',
                  },
                  { status: 400 }
                )
              }
                ...
                async start(controller) {
                  ...
                  try {
                    for (let port = startPortNum; port <= endPortNum; port++) {
                      ...
                      const result = await new Promise<PortScanResult>(
                        resolve => {
        ...
        '/api/dns-lookup': {
          async POST(req) {
            ...
            try {
              ...
              console.log(
                `Starting DNS lookup for ${domain}, type: ${recordType}`
              )
              ...
              try {
                ...
                try {
                  switch (recordType.toUpperCase()) {
                    ...
                    case 'CNAME':
                      const cnameRecords =
                        await dns.promises.resolveCname(domain)
        ...
        '/api/device-discovery-stream': {
          async POST(req) {
            ...
            try {
              ...
              console.log(
                `Starting stream device discovery for ${networkRange}`
              )
                ...
                async start(controller) {
                  ...
                  try {
                    ...
                    for (let i = 0; i < maxHosts; i++) {
                      ...
                      console.log(
                        `Scanning host ${i + 1}/${maxHosts}: ${targetIp}`
                      )
                        ...
                        if (isOnline) {
                          ...
                          try {
                            const arpCmd = isWindows
                              ? `arp -a ${targetIp}`
                              : `arp -n ${targetIp}`
                            const { stdout: arpOutput } =
                              await execAsync(arpCmd)
                      ...
                      try {
                        controller.enqueue(
                          new TextEncoder().encode(progressData)
                        )
                      } catch {
                        console.log(
        ...
        '/api/snmp-browse': {
          async POST(req) {
            ...
            try {
              ...
              console.log(
                `Starting SNMP browse for ${target}, version: ${version}`
              )
              ...
              try {
                ...
                if (version === '3') {
                  ...
                  console.log(
                    `Creating SNMP session with community: ${community}`
                  )
                ...
                const varbinds = await Promise.race([
                  new Promise<any[]>((resolve, reject) => {
                    ...
                    session.get?.(systemOids, (error: any, varbinds: any) => {
                      ...
                      if (error) {
                        reject(
                          new Error(error?.message || 'SNMP request failed')
                        )
                      } else {
                        resolve(varbinds || [])
                      }
                  ...
                  new Promise<any[]>((_, reject) => {
                    ...
                    setTimeout(() => {
                      session.close?.()
                      reject(
                        new Error('SNMP request timed out after 5 seconds')
                      )
                    }, 5000)
                  }),
                ])
                ...
                for (const varbind of varbinds) {
                  if (varbind && !snmp.isVarbindError?.(varbind)) {
                    mibs.push({
                      ...
                      type: varbind.type || 0,
                      description:
                        oidDescriptions[varbind.oid] || 'SNMP object',
                      hasChildren: false,
                    })
                  }
                }
                ...
                const commonGroups = [
                  ...
                  },
                  {
                    ...
                    value: '',
                    description:
                      'Interfaces group - network interface information',
                    hasChildren: true,
                  },
                  {
                    oid: '1.3.6.1.2.1.25',
                ...
                if (
                  ...
                  errorMessage =
                    'Connection refused - SNMP service not available'
                  suggestions = [
      ...
      fetch(req) {
        ...
        if (
          ...
          return new Response(
            Bun.file(
              path.join(
                publicDir,
                'network-administration/inventory/index.html'
              )
            )
          )
        }
        ...
        if (
          pathname === '/tools' ||
          pathname === '/network-administration/tools'
        ) {
          return new Response(
            Bun.file(
        ...
        if (pathname === '/network-administration/tools/ping') {
          return new Response(
            Bun.file(
              path.join(
                publicDir,
                'network-administration/tools/ping/index.html'
              )
            )
          )
        }
        ...
        if (pathname === '/network-administration/snmp') {
          return new Response(
            Bun.file(
              path.join(publicDir, 'network-administration/snmp/index.html')
            )
          )
        }
        ...
        if (pathname === '/network-administration/snmp/oid') {
          return new Response(
            Bun.file(
              path.join(publicDir, 'network-administration/snmp/oid/index.html')
            )
          )
        }
        ...
        if (pathname === '/network-administration/icmp/polling') {
          return new Response(
            Bun.file(
              path.join(
                publicDir,
                'network-administration/icmp/polling/index.html'
              )
            )
          )
        }
