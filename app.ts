...
for (let i = 0; i < MAX_PORT_ATTEMPTS; i++) {
  try {
    ...
    if (i === MAX_PORT_ATTEMPTS - 1) {
      ...
      console.error(`Failed to start WebSocket server after ${MAX_PORT_ATTEMPTS} attempts.`);
      console.error("Please check if multiple instances are running or specify a different port with WS_PORT env variable.");
      process.exit(1);
    }
...
const server = serve({
  ...
  fetch(req) {
    ...
    if (pathname === "/network-administration") {
      return new Response(Bun.file(path.join(publicDir, "network-administration.html")));
    }
    ...
    if (pathname === "/network-administration/icmp/polling/device-status" ||
      ...
      return new Response(Bun.file(path.join(publicDir, "network-admin-icmp-status.html")));
    }
